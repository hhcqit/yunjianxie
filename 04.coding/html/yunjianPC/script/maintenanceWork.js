Vue.component('my-content', {
    ...
    methods: {
                    ...
                    .then(function (willDelete) {
                        if (willDelete) {
                            var postData = {
                                ...
                                "content": $('#addForm [name="content"]').val(),//保养内容
                                "name": $('#addForm [name="equipId"] option:selected').html() + getNowFormatDate() + "适时保养",//任务名称：设备名称yyyymmdd适时保养,
                                "type": 1//1保养计划 2检定计划
                            };
        ...
        initCategoryDataTables: function () {
            ...
            _self.datatable_obj = $('#merchant-list').DataTable({
                ...
                columns: [
                    ...
                    },//保养状态0、未开始 1、执行中 2、待确认 3、已完成 ,
                    { data: 'executeUser' },//计划执行用户
                    {
                        data: 'executeTime',
        ...
        upd: function () {//执行保养
            ...
            if (_self.datatable_obj.rows('.selected').data().length > 0) {
                ...
                if(callbackData.status==3){
                    ...
                    $("#executeForm [name='content']").val(callbackData.content);
        ...
        FnExecuteSave: function () {
            ...
            if ($("[name='result']").val().trim() == "") {
                swal("", "保养说明不能为空", "error");
                return false
            } else if ($('.state.waiting').length != 0 || $('.state').length == 0) {
                ...
                $.ajax({
                    ...
                    data: JSON.stringify({
                        ...
                        // "certificateNo": _self.datatable_obj.row('.selected').data().certificateNo,
